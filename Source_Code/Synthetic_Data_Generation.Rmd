...
sds = syn(ods, m = 8, seed = my.seed)
sds$syn

# for (i in 1:8)
# {
#   print(class(sds$syn[i]))
# }
```
```{r}
as.data.frame(sds$syn[2])
```
...
```{r}
Generate_Synthetic_Data_For_All_Patients <- function(main.Path, M, my.Seed)
{
  ...
  for (i in 1:len)
  {
    ...
    writePath <- paste(writePath, file.Names[i], sep = "/")
    #print(writePath)
    write.csv(x = df.Out, file = writePath, row.names = FALSE)
  }
}
...
Generate_Synthetic_Data_For_All_Patients(main.Path, M, my.Seed)
