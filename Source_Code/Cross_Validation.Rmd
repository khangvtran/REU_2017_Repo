...
KFolds_Cross_Validate <- function(main.Path, K, M, vars, my.Seed, is.Group = FALSE)
{
  set.seed(my.Seed)
...
data <- read.csv(file = "/Users/KVTran/Documents/Research/WPI/Slip_Buddy/REU_2017/data/Individual_Analysis/Result/patientID_18/Result.csv")

X <- as.numeric(data[,1])
Y <- data[,2]
CL = 0.95
mu <- 0
#t.test(x = X, y = Y, mu = 0, alternative = "two.side" , paired = TRUE, conf.level = CL)
...
T_Test <- function(X, Y, mu, CL)
{
  ...
  Critical.Value <- qt(upper.Qt, df = df)
  # print(df)
  # print(d.Bar)
  # print(SE)
  ...
  result <- data.frame(CI.UpperBound, CI.LowerBound, conclusion)
